...
for fmri_nii = {'fmri1_nii','fmri2_nii','fmri3_nii','fmri4_nii'}
    ...
    if strcmp(fmri_nii,'NONE')
        inp.(['sw' fmri_nii]) = 'NONE';
    else
        ...
        job.comp{1}.def = {inp.deffwd_nii};
        job.comp{2}.id.space = {which(inp.refimg_nii)};
        job.out{1}.pull.fnames = {inp.(fmri_nii)};
        job.out{1}.pull.savedir.saveusr = {inp.out_dir};
        job.out{1}.pull.interp = 1;
        job.out{1}.pull.mask = 0;
        job.out{1}.pull.fwhm = [0 0 0];
        ...
        [~,n,e] = fileparts(inp.(fmri_nii));
        inp.(['w' fmri_nii]) = fullfile(inp.out_dir,['w' n e]);

        % Smooth
        clear matlabbatch
        matlabbatch{1}.spm.spatial.smooth.data = {inp.(['w' fmri_nii])};
        ...
        [~,n,e] = fileparts(inp.(['w' fmri_nii]));
        inp.(['sw' fmri_nii]) = fullfile(inp.out_dir,['s' n e]);
